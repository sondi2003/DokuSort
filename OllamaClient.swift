...
final class OllamaClient {
    static func suggest(from text: String, baseURL: String, model: String, promptTemplate: String) async throws -> Suggestion {
        guard let url = URL(string: "\(baseURL)/api/generate") else {
            throw OllamaError.invalidURL
        }

        // Ersetze {TEXT} Platzhalter im Prompt-Template mit dem tatsächlichen Text
        let prompt = promptTemplate.replacingOccurrences(of: "{TEXT}", with: String(text.prefix(4000)))
